...
class GenEC(pl.LightningModule):
    ...
    def train_RL(self, batch):          # Policy Gradient with Baseline算法
        gold_inputs_sentences = [self.input_formater.format_input(example=example)[-1] for example in batch]
        gold_output_sentences = [self.oupt_formater.format_output(example=example) for example in batch]

        # compute log_probs, get sample outputs
        task_prefix = 'causality identification'
        inputs_encoding_for_generate = self.tokenizer_for_generate([f"{task_prefix}\n{sent}" for sent in gold_inputs_sentences],
                                                                    ...
                                                                    truncation=True,
                                                                    return_tensors="pt")
        # generate sequences using beam search sampling
        ...
        with torch.no_grad():
            greedy_outputs = self.t5.generate(input_ids=inputs_encoding_for_generate.input_ids.cuda(), 
                                            do_sample=False, 
                                            top_k=20, 
                                            top_p=0.95, 
                                            max_length=self.hparams.max_oupt_len, 
                                            num_return_sequences=1, 
                                            num_beams=1,)
            greedy_seqs = self.tokenizer_for_generate.batch_decode(greedy_outputs, skip_special_tokens=True)
        ...
        advantages = sample_reward - baseline_reward
        baseline_values = torch.mean(sample_reward).repeat(len(advantages))
        policy_gradient_loss = -torch.mean(log_probs * advantages)
